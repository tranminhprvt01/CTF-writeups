...
class Desfunctional:
    ...
    def get_challenge(self):
        ...
        print(self.challenge.hex(), "original pt")
        return cipher.encrypt(self.challenge)

    def corruption(self):
        print(len(self.flipped_bits))
        if len(self.flipped_bits) == 192:
            self.flipped_bits = set(range(0, 192, 8))
            print("bruh it reset" , len(self.flipped_bits))
        ...
        print(len(self.flipped_bits), sorted(list(self.flipped_bits))[:24], sorted(list(self.flipped_bits))[:24] == list(range(0, 24)))
        mask = int.to_bytes(sum(2**i for i in self.flipped_bits), 24, 'little')
        print(self.key.hex(), "original key")
