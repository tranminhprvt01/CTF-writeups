...
def decrypt_and_parse_save_data(key, nonce, save_data, tag):
    ...
    print(character.nickname, character.day, character.stamina, character.intelligence, character.friendship, "after dec")
    return character
...
def load_game(ID, PW):
    ...
    # read save file
    try:
        with open(f'server/save/{file_name}', 'rb') as f:
            raw_data = f.read()
...
def save_game(ID, PW, character, save_data, tag):
    ...
    try:
        ...
        if character.day != character_parse.day or \
           ...
           character.friendship != character_parse.friendship:
            print("IM IN HERE WHICH MEAN SAVE FAILED :(")
            return SAVE_FAIL
    ...
    try:
        with open(f'server/save/{file_name}', 'wb') as f:
            f.write(save_data)
            f.write(tag)
    except:
