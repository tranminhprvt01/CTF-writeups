

# This file was *autogenerated* from the file solve.sage
from sage.all_cmdline import *   # import sage library

_sage_const_2 = Integer(2); _sage_const_128 = Integer(128); _sage_const_7 = Integer(7); _sage_const_1 = Integer(1); _sage_const_8 = Integer(8); _sage_const_0 = Integer(0); _sage_const_32 = Integer(32); _sage_const_16 = Integer(16); _sage_const_12 = Integer(12); _sage_const_3 = Integer(3)
from Crypto.Cipher import AES
import os
os.environ['PWNLIB_NOTERM'] = '1'
os.environ['TERM'] = 'xterm-256color'

from pwn import *
from Crypto.Util.number import long_to_bytes, bytes_to_long
from Crypto.Util.Padding import pad, unpad
import struct

R = PolynomialRing(GF(_sage_const_2 ), 'x', names=('x',)); (x,) = R._first_ngens(1)
GHASH_modulus = x**_sage_const_128  + x**_sage_const_7  + x**_sage_const_2  + x + _sage_const_1 
K = GF(_sage_const_2 **_sage_const_128 , name='a', modulus=GHASH_modulus)


def bytes_to_bit_array(data):
    bit_array = []
    for byte in data:
        bits = bin(byte)[_sage_const_2 :].zfill(_sage_const_8 )  
        bit_array.extend(map(int, bits))  
    return bit_array

def bit_array_to_bytes(bit_array):
    pad_length = len(bit_array) % _sage_const_8 
    if pad_length != _sage_const_0 :
        bit_array.extend([_sage_const_0 ] * (_sage_const_8  - pad_length))
    bytes_data = bytearray()
    for i in range(_sage_const_0 , len(bit_array), _sage_const_8 ):
        byte = _sage_const_0 
        for j in range(_sage_const_8 ):
            byte |= bit_array[i + j] << j
        bytes_data.append(byte)

    return bytes(bytes_data)


def encrypt_gcm(key, iv, pt,aad = None):
    cipher = AES.new(key, AES.MODE_GCM, nonce=iv)
    if aad:
        cipher = cipher.update(aad)
    ct, tag = cipher.encrypt_and_digest(pt)
    return ct, tag
def decrypt_gcm(key, iv, ct, tag, aad = None):
    
    cipher = AES.new(key, AES.MODE_GCM, nonce=iv)
    if aad:
        cipher = cipher.update(aad)
    pt = cipher.decrypt_and_verify(ct, tag)
    return pt



def encrypt_ECB(key, pt):
    cipher = AES.new(key, AES.MODE_ECB)
    ct = cipher.encrypt(pt)
    return ct

def encrypt_EBC_nonce(key,pt,iv):
    cipher = AES.new(key, AES.MODE_ECB)
    key_expansion = cipher.encrypt(iv)
    ct = xor(pt,key_expansion)
    return ct


def bytes_to_field_element(byte_array):
    bin_arr = bytes_to_bit_array(byte_array)
    return K(bin_arr)

import time
def field_element_to_bytes(field_element):
    bit_array = field_element.list()
    byte_data = bytearray()
    for i in range(_sage_const_0 , len(bit_array), _sage_const_8 ):
        bits = bit_array[i:i+_sage_const_8 ]
        byte_str = ''.join(str(bit) for bit in bits)
        byte = int(byte_str, _sage_const_2 )
        byte_data.append(byte)
    return bytes(byte_data)



def length_block(aad_length, ciphertext_length):
    aad_length_bits = aad_length * _sage_const_8 
    ciphertext_length_bits = ciphertext_length * _sage_const_8 
    
    # Pack the lengths into a byte array using big-endian format
    last_block = struct.pack('>QQ', aad_length_bits, ciphertext_length_bits)
    return last_block



def solve(message,TAG,aad=None):
    key = os.urandom(_sage_const_32 )
    numberofblock = _sage_const_0 
    message = pad(message,_sage_const_16 )
    numberofblock = numberofblock + len(message)//_sage_const_16 
    iv = os.urandom(_sage_const_12 )
    iv = bytes(iv)
    array_ct_fixed = []
    if aad:
        for i in range(_sage_const_0 ,len(aad),_sage_const_16 ):
            array_ct_fixed.append(aad[i:i+_sage_const_16 ])
    else :
        array_ct_fixed = []
    for i in range(_sage_const_2 , numberofblock+_sage_const_2 ):
        ct = encrypt_EBC_nonce(key,message[(i-_sage_const_2 )*_sage_const_16 :(i-_sage_const_1 )*_sage_const_16 ],iv+b'\x00'*_sage_const_3 +bytes([i]))
        array_ct_fixed.append(ct)
    
    H1 = encrypt_ECB(key,b'\x00'*_sage_const_16 )
    
    # nonce for T = iv||0^31||1
    T1 = encrypt_EBC_nonce(key,b'\x00'*_sage_const_16 ,iv+b'\x00'*_sage_const_3 +b'\x01')

    LENBLOCK = length_block(_sage_const_0 ,len(message)+_sage_const_16 ) # 16 last append block :> 
    if aad:
        LENBLOCK = length_block(len(aad),len(message)+_sage_const_16 ) # 16 last append block :>
    LENBLOCK =bytes_to_field_element(LENBLOCK)
   
    H1 = bytes_to_field_element(H1)

    T1 = bytes_to_field_element(T1)
    array_ct = [bytes_to_field_element(i) for i in array_ct_fixed]

    TAG = bytes_to_field_element(TAG)
    LHS = H1**_sage_const_2 
    RHS = sum([H1**(len(array_ct)-(i)+_sage_const_2 )*array_ct[i] for i in range(len(array_ct))]) + (H1)*LENBLOCK + T1 
    
    # we have TAG = LHS*X + RHS
    Need_block = (TAG-RHS)*LHS**(-_sage_const_1 )
    
    
    ciphertext_attack = iv
    if aad:
        array_ct = array_ct[len(aad)//_sage_const_16 :]
    for i in range(_sage_const_0 ,len(array_ct)):
        ciphertext_attack += field_element_to_bytes(array_ct[i])
    ciphertext_attack += field_element_to_bytes(Need_block)
    ciphertext_attack += field_element_to_bytes(TAG)
    return key,ciphertext_attack
    


# io = process(['python3','chall.py'])
# print(io.recvuntil(b'tag: '))
# TAG = io.recvline().strip()
# TAG = bytes.fromhex(TAG.decode())
# print(io.recvuntil(b'aad: '))
# aad = io.recvline().strip()
# aad = bytes.fromhex(aad.decode())
# print(io.recvuntil(b'need_message: '))
# message = io.recvline().strip()
# message = bytes.fromhex(message.decode())


message = bytes.fromhex('')
tag = bytes.fromhex('4a86c1b3842857ad6fef88d9c08d2cdd')

key,ciphertext_attack = solve(message,tag)


print(key)
print(ciphertext_attack)

print(key.hex())
print(ciphertext_attack.hex())

